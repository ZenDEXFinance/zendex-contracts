...
import "@oz4.8.1/contracts/access/Ownable.sol";
import "@oz4.8.1/contracts/security/ReentrancyGuard.sol";
import "@oz4.8.1/contracts/token/ERC20/IERC20.sol";
import "@oz4.8.1/contracts/token/ERC20/utils/SafeERC20.sol";
...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice Address of QDX contract.
    IERC20 public immutable QDX;
    ...
    uint256 public latestPeriodQDXPerSecond;
    ...
    /// @notice Record the QDX amount belong to MasterChefV3.
    uint256 public qdxAmountBelongToMC;
    ...
    /// @param _QDX The QDX token contract address.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20 _QDX, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        QDX = _QDX;
        nonfungiblePositionManager = _nonfungiblePositionManager;
        WETH = _WETH;
    }

    /// @notice Returns the QDX per second , period end time.
    /// @param _pid The pool pid.
    /// @return qdxPerSecond QDX reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256 qdxPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            qdxPerSecond = (latestPeriodQDXPerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice Returns the QDX per second , period end time. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return qdxPerSecond QDX reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256 qdxPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            qdxPerSecond =
                (latestPeriodQDXPerSecond * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) /
                totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice View function for checking pending QDX rewards.
    /// @dev The pending QDX amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    /// @param _tokenId Token Id of NFT.
    /// @return reward Pending reward.
    function pendingQDX(uint256 _tokenId) external view returns (uint256 reward) {
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (QDX.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    /// @notice Update the given pool's QDX allocation point. Can only be called by the owner.
    ...
    /// @notice harvest QDX from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return reward QDX reward.
    ...
    function transferToken(address _token, address _to) internal {
        ...
        // Need to reduce qdxAmountBelongToMC.
        if (_token == address(QDX)) {
            unchecked {
                // In fact balance should always be greater than or equal to qdxAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balance >= qdxAmountBelongToMC) {
                    balance -= qdxAmountBelongToMC;
                } else {
                    // This should never happend.
                    qdxAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        // Need to reduce qdxAmountBelongToMC.
        if (token == address(QDX)) {
            unchecked {
                // In fact balance should always be greater than or equal to qdxAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balanceToken >= qdxAmountBelongToMC) {
                    balanceToken -= qdxAmountBelongToMC;
                } else {
                    // This should never happend.
                    qdxAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of QDX injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        // Transfer QDX token from receiver.
        QDX.safeTransferFrom(receiver, address(this), _amount);
        // Update qdxAmountBelongToMC
        unchecked {
            qdxAmountBelongToMC += _amount;
        }
        ...
        uint256 qdxPerSecond;
        uint256 qdxAmount = _amount;
        if (latestPeriodEndTime > currentTime) {
            uint256 remainingQDX = ((latestPeriodEndTime - currentTime) * latestPeriodQDXPerSecond) / PRECISION;
            emit UpdateUpkeepPeriod(latestPeriodNumber, latestPeriodEndTime, currentTime, remainingQDX);
            qdxAmount += remainingQDX;
        }
        qdxPerSecond = (qdxAmount * PRECISION) / duration;
        unchecked {
            latestPeriodNumber++;
            latestPeriodStartTime = currentTime + 1;
            latestPeriodEndTime = endTime;
            latestPeriodQDXPerSecond = qdxPerSecond;
        }
        emit NewUpkeepPeriod(latestPeriodNumber, currentTime + 1, endTime, qdxPerSecond, qdxAmount);
    }

    /// @notice Update QDX reward for all the liquidity mining pool.
    ...
    /// @notice Update QDX reward for the liquidity mining pool.
    ...
    /// @notice Safe Transfer QDX.
    /// @param _to The QDX receiver address.
    /// @param _amount Transfer QDX amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = QDX.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            // Update qdxAmountBelongToMC
            unchecked {
                if (qdxAmountBelongToMC >= _amount) {
                    qdxAmountBelongToMC -= _amount;
                } else {
                    qdxAmountBelongToMC = balance - _amount;
                }
            }
            QDX.safeTransfer(_to, _amount);
        }
    }
